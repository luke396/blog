...
2. **计算损失函数**（考虑 L2 正则化）：
   $$J^{\{t\}} = \frac{1}{1000}\sum_{i=1}^{1000}l(\hat{y}^{(i)}, y^{(i)}) + \frac{\lambda}{2 \times 1000}\sum_{l}\|W^{[l]}\|^{2}_{F}$$
...
**Frobenius 范数/norm**：$\lVert A \rVert_{\mathrm{F}}=\sqrt{\sum_{i=1}^{m}\sum_{j=1}^{n}|a_{ij}|^{2}}$，是矩阵所有元素的平方根。灵感来源于 RMS[^7]，乍一看很像 L2 norm 或者 2-norm，但其实并不能完全认为二者等价 [^8]。
